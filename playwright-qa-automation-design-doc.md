...
- `staging` — where the P5 patch plan ran, post-deploy regression target.
- `production-readonly` (or however we scope safe production checks) — mirrors what the Admin QA Tracker calls "New Production," used for read-mostly/config-verification checks only; anything mutating (orders, customer creation) should default to staging unless there's an explicit, controlled reason to run against production, matching the "controlled test account" discipline already used manually.
...
- [ ] Confirm environment list and base URLs (staging confirmed via P5 plan; production-readonly scope needs explicit sign-off on what's safe to touch).
...
- [ ] Confirm if there's a known seasonal/promotional peak-traffic window to target for k6 load scenarios.
...
2. Scaffold the repo per §6, get one trivial smoke test green in CI against staging.
3. Migrate a first vertical slice from the trackers end-to-end (recommend: Registration/Login, since it's self-contained and already 100% Pass in the P5 plan — good for validating the framework itself before tackling integration-heavy areas).

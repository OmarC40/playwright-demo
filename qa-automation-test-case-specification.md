...
|---|---|---|---|
| REG-001 | New customer registration, all valid fields | Medium | 🟢 |
| REG-002 | Registration with missing required fields | Medium | 🟢 |
...
3. Get staging access and run `playwright codegen` to start capturing real selectors.
4. Begin implementation with the three cross-referenced "good first target" clusters: **Registration/Login (REG-)**, **Order Minimums (MIN-)**, and **Percent Shipping (PS-)** — all high-detail, low-ambiguity, and low external-integration risk.
